function Feat_Index =  PIMA_Evolutiva
clear all
global Data 
clear
clc
...
load PIMA.data
Data  = load('PIMA.data'); % Se carga el dataset
LargoString =8; % Número de características en el dataset
...
ParametrosGA = gaoptimset('CreationFcn', {@PopFunction},...
                     ...
                     'Display', 'iter'); 
rand('seed',1)
nVars = 8; % Cantidad de variables independientes
...
function [FitVal] = FitFunc_SVM(pop)
    ...
    columnasSeleccionadas=PIMA(1:end,ind);
    clasificaciones=PIMA(1:end,9);
